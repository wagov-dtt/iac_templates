...
commands = [
    ...
    f"az vm create -g {name} -n {name}-rancher --image {vm_image} --size {vm_size} --public-ip-sku Standard --public-ip-address-dns-name {dns_name} --custom-data cloud-init.txt --assign-identity --role contributor --scope /subscriptions/$(az account show --query id -o tsv)/resourceGroups/{name}",
...
for attempt in range(30):  # 10 minutes total (30 * 20 seconds)
    try:
